...
class Transformer(t2t_model.T2TModel):
  ...
  def encode(self, inputs, target_space, hparams, features=None, losses=None):
    ...
    s = common_layers.shape_list(encoder_decoder_attention_bias)
    s1 = common_layers.shape_list(encoder_output)
    return encoder_output, tf.zeros([s[0],s[1],s[2],s1[1]//hparams.hidden_size], dtype=encoder_output.dtype) #encoder_decoder_attention_bias
    #return encoder_output, encoder_decoder_attention_bias

  def mydecode(self, encoder_output, hparams):
        return my_decoder(
        encoder_output,
        hparams)
  ...
  def body(self, features):
    ...
    encoder_output = self.mydecode(encoder_output, hparams)
    ...
    if self.has_input:
      ...
      with tf.variable_scope("body"):
        ...
        def slerp(z0, z1, t):
            ...
            dot = tf.reduce_sum(z0*z1)
            z1 = z1*tf.sign(dot)
            dot = dot*tf.sign(dot)

            def r1():
                result = z0 + t * (z1 - z0)
                result = tf.nn.l2_normalize(result, axis=0, epsilon=1e-8)
                return result
            ...
            def r2():
                ...
                return (tf.cos(theta) - dot * sin_theta / so) * z0 + sin_theta / so * z1

            DOT_THRESHOLD = 0.9995
            result = tf.cond(dot > DOT_THRESHOLD, r1, r2)
            return result

        def analogy(z0, z1, z2):
            result = z0-z1+z2
            result = tf.nn.l2_normalize(result, axis=0, epsilon=1e-8)
            return result
        ...
        z0 = encoder_output[0][0]

        #interp
        #z1 = encoder_output[0][-1]
        #length = 7 #encoder_output[0].shape[0]
        #iss = [slerp(z0,z1,x/(length-1)) for x in range(length)]
        #encoder_output = [tf.stack(iss)]

        #analogy
        #z1 = encoder_output[0][1]
        #z2 = encoder_output[0][2]
        #z3 = analogy(z0,z1,z2)
        #encoder_output = [tf.stack((z3,z3,z3))]

        #additive compositionality
        #z1 = encoder_output[0][1]
        #z2 = addcomp(z0,z1)
        #encoder_output = [tf.stack((z2,z2))]
...
def transformer_encoder(encoder_input,
  ...
  with tf.variable_scope(name):
    ...
    #s = common_layers.shape_list(x)
    x = tf.reduce_sum(x, axis=1) #/tf.sqrt(tf.cast(s[1], x.dtype))

    #a = tf.layers.dense(x, hparams.eyal_hdims, activation=tf.tanh)
    #a = tf.layers.dense(a, hparams.eyal_heads, activation=tf.nn.softmax)

    
    #x = tf.matmul(tf.transpose(a,[0,2,1]),x)

    #x = tf.layers.flatten(x)

    x = tf.nn.l2_normalize(x, axis=-1, epsilon=1e-8)
    
    x = tf.identity(x, name='fixed_code')
    return x


def my_decoder(encoder_output, hparams):
      #encoder_output = tf.tile(tf.expand_dims(encoder_output, axis=1),[1,hparams.eyal_heads,1])
      s = common_layers.shape_list(encoder_output)
      encoder_output = encoder_output + tf.random_normal(s, stddev=hparams.eyal_noise_dropout, dtype=hparams.activation_dtype)
      encoder_output = tf.nn.l2_normalize(encoder_output, axis=-1, epsilon=1e-8)      
      encoder_output = tf.reshape(encoder_output, (s[0], -1, hparams.hidden_size))
...
def transformer_base_v1():
  ...
  hparams.add_hparam("eyal_noise_dropout", 0.05)
  hparams.add_hparam("eyal_heads", 8)
  hparams.add_hparam("eyal_hdims", 64)
